...
Crawler.prototype._executeCrawlerRequest = function _executeCrawlerRequest (options) {
    ...
    console.log("crawlling\t"+options.uri);
...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest (options) {
    ...
    if (!ropts.headers) { ropts.headers = {'User-Agent':'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.6)Gecko/20091201 Firefox/3.5.6'}};
    if (ropts.forceUTF8) {
    ...
    //request(_.pick.apply(this,[ropts].concat(requestArgs))).pipe(fs.createWriteStream('doodle.png'))
    var req = request(_.pick.apply(this,[ropts].concat(requestArgs)), function(error,response) {
        ...
        if(options.debug)
            console.log('request'+_.pick.apply(this,[ropts].concat(requestArgs)))
...
Crawler.prototype._onContent = function _onContent (error, options, response, fromCache) {
    ...
    if (isHTML && options.jQuery && options.method !== 'HEAD') {
        ...
        var type = path.extname(options.uri);
        options.callback(null,response,type);
        self.emit('pool:release', options);
    }
};
