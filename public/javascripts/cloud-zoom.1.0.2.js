...
(function ($) {
    ...
    //////////////////用于浮层显示的函数。
    function notice_show(imgs)//显示
    {
        var images = JSON.parse(imgs);
        $('#main').css('opacity','0.6').attr('class','blur10 main');
        $('#slider-one').empty();
        for(var i=0;i<images.length;i++) {
            var odiv = $('<div>').appendTo($('#slider-one'));
            var oimg = $('<img>').attr('src', imgPre+images[i].img_url).attr('alt','').appendTo($(odiv));
        }
        //$('img').fancybox();
